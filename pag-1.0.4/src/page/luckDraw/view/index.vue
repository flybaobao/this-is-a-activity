<template>
  <div class="luckDraw" ref="luckDrawPage">
    ...
    <div class="share" v-if="flag && sharing" @click="shares">立即分享</div>
  </div>
</template>
...
export default {
  ...
  mounted () {
    ...
    this.sharing = utils.getUrlKey('share') === 'true' ? !this.share : this.share
    setTimeout(() => {
      if (this.sharing) {
        if (this.login !== '' && this.login !== null) {
          this.$native.share({
            name: '燕窝福利大抽奖',
            desc: '燕窝福利大抽奖 -人人参与，100%中奖-',
            url: `${HOST_ROUTER}/luckDraw.html?draw_prize_id=${this.num}&token=${this.login}&share=${this.sharing}`,
            pic: this.banner
          })
        } else {
          this.$native.goBack()
